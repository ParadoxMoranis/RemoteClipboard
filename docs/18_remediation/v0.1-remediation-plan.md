...
本次保留模块化单体方向，但收敛为以下关系：平台 GUI 只链接 `client_common`；CLI 继续使用已验证的 v1 实现；两个服务器入口链接同一个 `server_common` 源集，但拥有唯一 CMake target 和唯一产物名；`TcpServer` 仍是协调器，`FrameParser` 负责有界分帧，`IEventStorage` 使用可报告失败的持久化接口，`MemoryRouter` 只保存活跃连接。
...
3. `IEventStorage::recordEvent` 改为返回结果（成功、可重试失败、永久失败），服务端仅在持久化语义满足时确认事件。v0.1 可采用“路由继续、日志失败可观测”的策略，但必须记录结构化错误；不可伪装为已持久化。
...
classDiagram
  ...
  class IEventStorage {+recordEvent() StorageResult}
  class IRouter {+register() +unregister() +broadcast()}
  TcpServer --> Session
  Session --> FrameParser
  TcpServer --> IEventStorage
  TcpServer --> IRouter
```
